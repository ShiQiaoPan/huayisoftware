...
@interface WPFindPwdViewController ()<UITextFieldDelegate>{
...
@implementation WPFindPwdViewController
...
- (void)respondsToGetPwdBtn {
    ...
    if (![self.pwdTextField.text isEqualToString:self.rePwdTextField.text]) {
        ...
        MBProgressHUD * getHud = [MBProgressHUD showHUDAddedTo:self.view animated:YES];
        getHud.color = [UIColor grayColor];
        getHud.removeFromSuperViewOnHide = YES;
        getHud.labelText = @"请稍等";
        getHud.detailsLabelText = @"正在重置密码...";
        __weak typeof(self)weakself = self;
        [NetWorkingManager reSetPwdWithName:self.phoneTextField.text code:self.testNumTextField.text newpassword:self.pwdTextField.text newpassword2:self.rePwdTextField.text successHandler:^(id responseObject) {
            if ([responseObject[@"success"] integerValue]) {
                getHud.labelText = @"重置密码成功";
                getHud.detailsLabelText = nil;
                ...
                if (responseObject[@"errMsg"]) {
                    getHud.labelText = responseObject[@"errMsg"];
                } else {
                    getHud.labelText = @"重置密码失败";
                }
                getHud.detailsLabelText = nil;
                [getHud hide:YES afterDelay:1.0];
            }
            
        } failureHandler:^(NSError *error) {
            getHud.labelText = @"网络错误";
            getHud.detailsLabelText = error.localizedDescription;
            [getHud hide:YES afterDelay:2.0];
        }];
}
